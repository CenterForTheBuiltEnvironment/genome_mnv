...
seq_plot <- function(df_means, sprt_res, sprt_overlap_base, sprt_overlap_interv, annual_saving, true_saving, eob){
  
  #' @description This is a function defined to plot the overal results from sequential analysis. 
  #' @usage seq_plot(df_means, sprt_res, sprt_overlap_base, sprt_overlap_interv, annual_saving, true_saving, eob)
  #' @param df_means running weekly mean
  #' @param sprt_res sequential probability ratio test results
  #' @param sprt_overlap_base baseline weather condition overlap percentage with tmy
  #' @param sprt_overlap_interv intervention weather condition overlap percentage with tmy
  ...
  p2 <- ggplot(sprt_res, aes(x = n_weeks, y = ns_stat)) +
    ...
    scale_x_continuous(expand = c(0, 0), 
                       limits = c(1, sprt_param$n_weeks + 0.5),
  ...
  p4 <- ggplot(sprt_overlap_base) +
    geom_line(aes(x = n_weeks, y = overlap_base, color = "Baseline"), 
              size = 1.25) +
    geom_line(data = sprt_overlap_interv, 
              aes(x = n_weeks, y = overlap_interv, color = "Intervention"),
              size = 1.25) +
  ...
  p1 <- ggplot(df_means, aes(x = week, y = value_ave)) +
    ...
    geom_text(data = filter(df_means, 
                            week == sprt_param$n_weeks, 
                            parameter == sprt_param$parameter), 
              aes(x = sprt_param$n_weeks, 
                  y = value_ave, 
                  color = strategy, 
                  label = strategy), 
              position = position_nudge(x = 0.5), 
  ...
  p3 <- ggplot(annual_saving) +
    ...
    geom_text(data = annual_saving %>% filter(n_weeks == TC), 
              ...
              size = 2.5, 
              fontface = "italic") +
    geom_point(data = annual_saving %>% filter(n_weeks == block_params$block_unit), 
               aes(x = n_weeks, y = annual_saving), 
               size = 4,
               alpha = 0.8, 
               shape = 16, 
               color = "#347EB3") +
    geom_text(data = annual_saving %>% filter(n_weeks == block_params$block_unit),
              aes(x = n_weeks, y = annual_saving,label = paste0("MinC: ", n_weeks)),
              position = position_nudge(y = -2.5),
              color = "grey20", 
              size = 2.5, 
              fontface = "italic")+
    ...
    geom_text(data = annual_saving %>% filter(n_weeks == eob),
              ...
              size = 3, 
              fontface = "bold")+
    geom_text(data = sprt_res %>% filter(n_weeks%%block_params$block_unit == 0 & flag == 1) %>% slice(1),
              aes(x = n_weeks, y = annual_saving, label = paste0("SPRT: ", n_weeks)), 
              position = position_nudge(y = 2.5),
              color = "grey20", 
              size = 2.5, 
              fontface = "italic") +
