...
```{r meanerr, fig.cap="Distribution of estimated deviation (without absolute calculation) from the ground-truth savings calculated by the two M&V methods (red dashed line highlighting 0% error)", fig.height=7, fig.width=9}
...
```{r absnull, fig.cap="Comparison between the two M&V methods in detecting no intervention effect when buildings are subject to baseline change.", fig.height=4, fig.width=9}
#### ABS-NULL ####
...
df_MW_A %>% 
  ...
  labs(fill = NULL, 
       ...
       title = "Estimated absolute effect of non-routine events by different M&V methods", 
...
```{r abscont, fig.cap="Comparison of different sampling ratio impact on M&V estimation accuracy over the entire 24 months (previous results of sampling at 50%/50% shown on the left side of the red dashed line)", fig.height=7, fig.width=9}
...
```{r stablecont, fig.cap="Comparison of different sampling ratio impact on M&V estimation accuracy using buildings with stable electricity usage over the entire 24 months (results of sampling at 50%/50% shown on the left side of the red dashed line)", fig.height=7, fig.width=9}
...
p_top <- df_MW_S %>% 
  ...
  labs(fill = NULL, 
       x = NULL, 
       y = "Absolute error in fractional savings", 
       subtitle = str_glue("{S_building} buildings in the stable subset with measured weather conditions")) +
...
p_bottom <- df_TW_S %>% 
  ...
  labs(fill = NULL, 
       x = NULL, 
       y = "Absolute error in fractional savings", 
       subtitle = str_glue("{S_building} buildings in the stable subset with TOWT model and TMY weather")) +
  coord_cartesian(ylim = c(0, 8)) +
  theme(panel.grid.major.y = element_line(color = "grey80", linewidth = 0.25),
        legend.position = "none",
        plot.margin = margin(t = 2, r = 7, b = 2, l = 2, unit = "mm"))
...
ggarrange(p_top, p_bottom, 
          ncol = 1, nrow = 2,
          labels = c("a)", "b)")) +
  plot_annotation(title = "Absolute error in savings estimation by different sampling ration(after 2 years)")
```


```{r meancont, fig.cap="Comparison of different sampling ratio impact on M&V estimation accuracy at the time of satisfying all stopping criteria (expressed without absolute calculation for errors)", fig.height=7, fig.width=9}
...
ggarrange(p_top, p_bottom, 
          ncol = 1, nrow = 2,
          labels = c("a)", "b)")) +
  plot_annotation(title = "Absolute error in savings estimation by different sampling ration\n(after satisfying all stopping criteria)")
```

```{r absint2y, fig.cap="Comparison of different sampling interval impact on M&V estimation accuracy over the course of two years (dropped: all non-consecutive days were dropped; kept: all measurements were kept)", fig.height=8, fig.width=9}
# dropped version
...
ggarrange(p_top, p_bottom, 
          ...
          common.legend = T, 
          legend = "bottom") +
  plot_annotation(title = "Absolute error in savings estimation of ranodmized M&V\n(after 2 years)")
```

```{r meanintspt, fig.cap="Comparison of different sampling interval impact on M&V estimation accuracy after satisfying all stopping criteria (without absolute calculation)", fig.height=8, fig.width=9}
...
ggarrange(p_top, p_bottom, 
          ...
          common.legend = T, 
          legend = "bottom") +
  plot_annotation(title = "Signed error in savings estimation of randomized M&V\n(after satisfying all stopping criteria)")

```

```{r meanint2y, fig.cap="Comparison of different sampling interval impact on M&V estimation accuracy over the course of the two years (without absolute calculation)", fig.width=9, fig.height=8}
# dropped version
df_drop <- df_interval_drop_stable %>% 
  bind_rows(df_interval_drop_variable) %>% 
  select(name, contains("fs")) %>% 
